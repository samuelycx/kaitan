...
export default function OrdersPage() {
        ...
        return (
          <div key={row.id} className="past-order">
            ...
            <div className="past-order-body">
              <p>{row.vendorName}</p>
              <small>
                {dayLabel(cstDate(row.at))} · {row.items.map((item) => `${item.name} × ${item.qty}`).join("、")}
              </small>
            </div>
  ...
  return (
    <Page>
      ...
      {done.length === 0 ? <Empty>还没吃过。点一单试试。</Empty> : history}
    </Page>
  );
}
